...
for each_restaurant in restaurant_list[1:]:
    ...
    businessLicenseHistory = getBusinessLicenseHistory(restaurant_name,required_restaurant_address)
            ...
            for key in each:
                ...
                if(key == 'business_activity'):
                    ...
                    if ('LIQUOR' in each['business_activity'].upper()):
                        ...
                        blockDetails = getCensusBlock(required_restaurant_address+' chicago IL')
...
def getCoordinates(value,numberOfBusiness):
    for i in range(0,numberOfBusiness):
        lat,lon = value[i]
        crimeHistory = getCrimeHistory(lat, lon)
        if (crimeHistory):
            return crimeHistory
